...
export default class JobGrid extends Component {
    ...
    onDetail(e) {
        ...
        this.onSetJobIdState(dataItem._id)
        if (this.state.jobId) {
            this.jobFormModal.onShowModal();
        } else {
            bootbox.alert('Tải lại trang và thử lại!.')
        }
    }

    onDelete(e) {
        const tr = $(e.currentTarget).closest('tr');
        const dataItem = this.grid.dataItem(tr);
        const _this = this;
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    $.blockUI();
                    axios.delete('/service/job/delete_job/' + dataItem._id).then(res => {
                        if (res.data.success) {
                            _this.onRefreshGrid();
        ...
        return (
            <div>
                <Grid
                    ...
                    {...this.gridOptions}
                />
                <JobFormModal ref={ref => this.jobFormModal = ref} onRefreshGrid={() => this.onRefreshGrid()} jobId={this.state.jobId} />
            </div>
        );
    }

    onSetJobIdState(sta) {
        this.state.jobId = sta;
